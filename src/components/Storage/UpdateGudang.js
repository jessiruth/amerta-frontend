...
const UpdateGudang = () => {
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Gagal memperbarui gudang.');
    } finally {
      setSaving(false);
      setShowConfirmation(false);
    }
      ...
      <div className="gudang-form-content">
        ...
        {showSuccessModal && (
            <div className="modal-overlay">
                <div className="modal-content">
                <div className="modal-header">
                    <h3>Gudang Diupdate</h3>
                    <button className="close-button" onClick={() => setShowSuccessModal(false)}>&times;</button>
                </div>
                <div className="modal-body">
                    <p>Gudang berhasil diupdate. Anda akan diarahkan ke detail gudang.</p>
                </div>
                <div className="modal-footer">
                    <button
