...
const UpdateGoods = () => {
  ...
  useEffect(() => {
    ...
    const fetchBarang = async () => {
      const res = await axiosInstance.get(`/api/barang/${id}`, {
      ...
      setHargaBeli(barang.hargaBeli.toString().replace(".", ","));
      setHargaJual(barang.hargaJual.toString().replace(".", ","));

      const mappedStock = barang.stockBarang.map(s => ({
        stock: s.stock,
        namaGudang: s.namaGudang,
        isExisting: true
      }));
      setStokList(mappedStock);
    };
  ...
  const validateForm = () => {
    ...
    if (!hargaBeli) {
      newErr.hargaBeli = "Harga beli wajib diisi";
    } else if (!beliValid) {
      newErr.hargaBeli = "Format harga beli tidak valid (gunakan koma, contoh: 12500,50)";
    } else if (beli < 1) {
      newErr.hargaBeli = "Harga beli minimal adalah 1";
    }

    if (!hargaJual) {
      newErr.hargaJual = "Harga jual wajib diisi";
    } else if (!jualValid) {
      newErr.hargaJual = "Format harga jual tidak valid (gunakan koma, contoh: 15000,75)";
    } else if (jual < 1) {
      newErr.hargaJual = "Harga jual minimal adalah 1";
    }

    if (isActive === null) newErr.isActive = "Status barang wajib dipilih";

    if (stokList.length === 0) {
      newErr.stokList = "Minimal harus ada satu stok barang";
    }
  ...
  const handleConfirm = async () => {
    ...
    try {
      await axiosInstance.put(`/api/barang/update/${id}`, {
        nama,
        kategori,
        merk,
        active: isActive,
  ...
  return (
    <div className="barang-form-container">
      ...
      <div className="barang-page-header">
        <h1 className="barang-page-title">Update Barang</h1>
      </div>
            ...
            <div className="modal-body">
              {showModal === "confirm" ? (
                ...
                <>
                  ...
                  <p className="warning-text">Semua perubahan yang belum disimpan akan hilang.</p>
                </>
              )}
            </div>
