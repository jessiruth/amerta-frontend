...
const ConfirmPurchaseOrder = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                if (!res.data?.data) throw new Error("Data PO tidak ditemukan");

                const order = res.data.data;
                setData(order);

                try {
                    const supplierRes = await axiosInstance.get(`/api/customer/${order.customerId}`, {
                        headers: { Authorization: `Bearer ${token}` },
                    });
                    setVendorName(supplierRes.data?.data?.name || "Unknown");
                } catch {
                    setVendorName("Unknown");
                }
            } catch {
                setData(null); // silent fail
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            setModalType(null); // optional: bisa tambahkan toast/snackbar error
        }
    };
